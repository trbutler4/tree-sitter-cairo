module.exports = grammar({
  ...
  rules: {
    ...
    _path_segment: ($) =>
      choice($.simple_path_segment, $.generic_argument_path_segment),
    simple_path_segment: ($) => seq(field("name", $.identifier)),
    ...
    // Tuple expressions
    tuple_expression: ($) =>
      seq("(", commaSep(field("member", $._expression)), ")"),

    // Function call expressions
    _arguments: ($) =>
      seq(commaSep1(field("argument", $._expression)), optional(",")),
    function_call_expression: ($) =>
      seq(field("name", $.path_expression), "(", optional($._arguments), ")"),
    ...
    single_struct_argument: ($) =>
      seq(field("name", $.identifier), $._struct_argument_expression),
    struct_argument_tail: ($) => seq("..", field("expression", $._expression)),
    _struct_argument: ($) =>
      choice($.single_struct_argument, $.struct_argument_tail),
    _struct_argument_list: ($) =>
      commaSep1(field("argument", $._struct_argument)),
    ...
    struct_pattern: ($) => seq(field("name", $.identifier), "with", ".."),
    _struct_patter_with: ($) =>
      seq(field("name", $.identifier), ":", field("value", $._pattern)),

    pattern_tuple: ($) => seq("(", commaSep(field("member", $._pattern)), ")"),
    ...
    _statement: ($) =>
      choice($.let_statement, $.expression_statement, $.return_statement),
    ...
    expression_statement: ($) =>
      choice($.match_expression, seq($._expression, ";")),
    ...
    param: ($) =>
      seq(
        optional($._modifier_list),
        field("name", $._param_name),
        $._type_clause,
      ),

    _implicits_clause: ($) =>
      seq("implicits", "(", commaSep(field("implicit", $.implicit)), ")"),

    implicit: ($) => seq(field("name", $.identifier), $._type_clause),
    ...
    module: ($) =>
      seq(repeat($.attribute), "mod", field("name", $.identifier), ";"),
    ...
    trait: ($) =>
      seq(
        ...
        field("name", $.identifier),
        optional($._wrapped_generic_parameters),
        choice(field("body", $._trait_body), ";"),
      ),

    _trait_body: ($) => seq("{", repeat($._trait_item), "}"),

    _trait_item: ($) => choice($._trait_function),
    ...
    impl: ($) =>
      seq(
        ...
        choice(field("body", $._impl_body), ";"),
      ),
    _impl_body: ($) => seq("{", repeat($._item), "}"),
    ...
    use: ($) =>
      seq(repeat($.attribute), "use", field("name", $.path_expression), ";"),

    generic_param: ($) => choice($.parenthesized_expression, $.identifier),
    _wrapped_generic_parameters: ($) =>
      seq("<", commaSep($.generic_param), ">"),
  },
});
